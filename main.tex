...
\begin{itemize}
    \item Weibliche(/männliche) Spraque-Dawley-Ratte \textcolor{red}{(wie schwer?)}. Narkotisiert mittels \textcolor{red}{...~ml Narcoren, davon ...~ml Babiturat} (0.3~ml/100~g, 16~\% Babiturat), in den unteren Bauchraum (intraperitoneal) im 45\degree Winkel injiziert.
\end{itemize}

Die Dosis stellt für das Tier eine starke Überdosis dar, die nach einiger Zeit zum Herzstillstand führen wird. Nach der Injektion folgt eine kurze Ruhephase für das Tier. \textcolor{red}{(wir mussten nochmal nachspritzen)}. Wenn es vollständig narkotisiert ist und keinen Zwischensehnenreflex mehr zeigt, wird mit der \textbf{Perfusion} begonnen. Dafür wird zunächst der Thorax geöffnet, um das noch schlagende Herz freizulegen. Die Haut wird vorsichtig mittig des Bauches von caudal nach rostral geöffnet. Anschließend wird das Zwerchfell aufgetrennt, die Rippen werden auf der linken und rechten Seite durchtrennt. Sobald das Herz freigelegt ist, wird mittels Spritze die linke Herzkammer geöffnet um eine Perfusionskanüle (Oliven-Kanüle) einzuführen. Diese wird mit einer Klemme befestigt. Danach wird die rechte Herzvorkammer mit einem Schnitt geöffnet. Mit etwa \textcolor{red}{150~ml 0.01m PBS} (Phosphate-buffered Saline, Raumtemperatur) wird das Herz-Kreislauf-System unter Verwendung einer Schlauchpumpe vorgespült. Dass sämtliches Blut ausgewaschen ist, ist am blass werdenden Gewebe erkennbar. Bei Ratten mit roten Augen ist dies am Farbwechsel der Augen von rot zu weiß erkennbar. Ist dies geschehen, wird das Gewebe mittels \textcolor{red}{400~ml 4~\%igem, eiskaltem Paraformaldehyd} fixiert. Durch chemische Reaktionen in den Muskeln kann es dabei zu Muskelzuckungen kommen. Die ausreichende Fixierung des Gewebes ist an einer stabilen Kopfposition beim Anheben des Tieres zu erkennen.
...
Nach er Fixierung folgt die \textbf{Gehirnentnahme}. Dazu wird der Kopf des Tieres abgetrennt. Anschließend wird die Haut dorsal entlang der Mittellinie aufgetrennt und die die Wirbelsäule umgebene Nackenmuskulatur entfernt. Mit einem Rangeur wird der Schädelknochen vorsichtig von caudal nach rostral entfernt, bis das Gehirn vollständig freigelegt ist. Nachdem alle Knochenrückstände vollständig entfernt wurden, kann das Gehirn nachfixiert werden. Zur \textbf{Nachfixierung} wird das Gehirn über Nacht in 4~\%igem Paraformaldehyd gegeben.

\subsection{Färbungen des Gewebes}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In Vorbereitung auf die Färbungen muss das Gehirn in dünne Scheiben geschnitten werden. Um Gewebeschädigungen durch den dazu erforderlichen Gefriervorgang zu vermeiden, ist ein \textbf{Gefrierschutz} erforderlich. Dafür wird das Gehirn in 20~\%ige Sucroselösung, 20~g Zucker gelöst in jeweils 70~ml Phosphatpuffer, aufgefüllt auf 100~ml, gelegt. Innerhalb dieser Lösung wird das Gehirn solange aufbewahrt, bis es auf den Boden abgesunken ist. Ist dies geschehen, wird der Vorgang mit 30~\%iger Sucroselösung wiederholt. Da dies einen Tag dauern kann, wird das Gehirn über das Wochenende in der Lösung gelassen. Zum \textbf{Schneiden} des Gehirns wird ein Gefriermikrotom verwendet. Um eine bessere Orientierung zu ermöglichen, wird eine Seite des Cortex mit einem länglichen Schnitt entlang der rostro-caudalen Achse gekennzeichnet. Anschließend wird das Gehirn in 50~$\upmu$m dicke Scheiben geschnitten. Bis zum Färbevorgang werden die Schnitte einzeln in mit TBS (Tris-buffered Saline) befüllten Mikroplates (im Kühlschrank) aufbewahrt. Jeder dritte Schnitt wird mittels unterschiedlicher Färbemethode eingefärbt. Für die Nissl- und Faser-Färbung werden jeweils vier Schnitte, in der richtigen Reihenfolge, auf Gelatine Chromalaun beschichtete Objektträger ausgezogen. Nachdem die Schnitte auf den Objektträgern getrocknet sind, kann mit dem Färbevorgang begonnen werden. \textcolor{red}{Die mittels immunhistochemischen Nachweises gefärbten Schnitte werden erst nach der Färbung auf die Objektträger aufgezogen.}

\subsubsection{Nissl-Färbung}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\textcolor{red}{Angeben: in Abb. Unterschrift mit N nummeriert}


\subsubsection{Faser-Färbung}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\textcolor{red}{Angeben: in Abb. Unterschrift mit F nummeriert}
...
\textcolor{red}{Angeben: in Abb. Unterschrift mit I? nummeriert}
...
Der Motorcortex befindet sich auf der Großhirnrinde, genauer gesagt liegt er auf dem Frontallappen. Anterior zu dem Sulcus centralis \index{Sulcus! centralis} der den Frontallappen vom Parietallappen abgrenzt befindet sich der Gyrus praecentralis \index{Gyrus! praecentralis}. Dieser enspricht dem Brodmann Areal 4 oder auch funktionell gesehen dem primären motorischen Cortex \index{primärer motorischen Cortex}. Innerhalb dieses Bereiches wird dabei die kontralaterale Körperhälfte auf eine somatotopische Art und Weise repräsentiert. Dabei ist die Darstellung des Körpers umgedreht. Dies bedeutet, dass der Kopf im untersten Bereich des Gyrus praecentralis und die unteren Extremitäten an der medialen Oberfläche der Hemisphäre direkt über dem Corpus callosum repräsentiert. Körperteile, die dabei besonders fein differenziert sind wie etwa die Hände, das Gesicht oder die Zunge nehmen dabei deutlich größere Bereiche des Kortex ein \textcolor{red}{Abbildung} \textsuperscript{\cite[13]{crossman2014neuroanatomy}}. Afferenzen erhält der Gyrus praecentralis hauptsächlich aus dem Nucleus ventralis anterolateralis, einer ventralen Kerngruppe des Thalamus. Dieses Kerngebiet leitet bereits vorverarbeitete motorische Impulse aus dem Cerebellum und den Basalganglien weiter. Efferenzen, die den primären motorischen Cortex verlassen gehören hauptsächlich zum Tractus corticospinalis. Damit kontrolliert der Gyrus praecentralis über diesen Faserstrang die Willkürmotorik. Eine weitere Efferenz, die diesen Bereich verlässt ist der Tractus corticonuclearis, welcher zu den Hirnnerven läuft und damit die Gesichtsmuskulatur steuert \textsuperscript{\cite[9]{trepel2011neuroanatomie}}. \\
Der Bereich anterior zum primären motorischen Cortex  wird auch als prämotorischer Cortex \index{prämotorischer Cortex} bezeichnet und entspricht dem Brodmann Areal 6. Die Region die sich dabei auf der medialen Oberfläche der Hemisphäre in diesem Bereich befindet wird als supplementärmotorischer Cortex \index{supplementärmotorischer Cortex} abgegrenzt. Beider Bereiche ähneln in ihrer Verschaltung dem primären motorischen Cortex. Ihre Aufgabe besteht aber höchstwahrscheinlich mehr darin Bewegungen zu programmieren und zu planen und die Körperhaltung zu kontrollieren \textsuperscript{\cite[13]{crossman2014neuroanatomy}}. 
